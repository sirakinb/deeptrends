...
interface ScheduleListProps {
  schedules: (QuerySchedule | ExtendedQuerySchedule)[];
...
export function ScheduleList({ schedules, onToggle, onDelete }: ScheduleListProps) {
  ...
  const groupedSchedules = new Map<string, (QuerySchedule | ExtendedQuerySchedule)[]>();
  ...
  const dedupedSchedules: (QuerySchedule | ExtendedQuerySchedule)[] = [];
    ...
    <div className="space-y-4">
      {dedupedSchedules.map((schedule) => {
        ...
        return (
          <div 
            key={schedule.id} 
            className={`card ${isPending ? 'border-2 border-blue-500 bg-blue-900/20 relative' : 'relative'}`}
          >
            {isPending && (
              <div className="absolute top-2 right-2 bg-blue-600 text-white text-xs px-2 py-1 rounded-md flex items-center shadow-md">
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  {schedule.frequency === 'weekly' && schedule.week_day && (
                    <div>
              ...
              <div className="flex items-center space-x-4">
                <Switch
                  ...
                  onChange={() => !isPending && onToggle(schedule.id)}
                  className={`${
                ...
                <button
                  onClick={() => !isPending && onDelete(schedule.id)}
